...
uint64 RookAttacks    [64];
uint64 BishopAttacks  [64];
...
uint64 fancy_magic_lookup_table[97264];

// Precomputed (factor, table_ptr) entries — filled in by MoveGeneratorBitboard::init().
FastMagicEntry bishop_magics_fast[64];
FastMagicEntry rook_magics_fast[64];

// E18: same data, split into two parallel scalar arrays. MSVC loads the 16-byte
// FastMagicEntry as XMM and then needs movq+psrldq+movq to get factor and ptr
// into GPRs (4 µops). Two scalar loads issue in parallel, no XMM round-trip.
uint64  bishop_magic_factors[64];
uint64 *bishop_magic_tables [64];
uint64  rook_magic_factors  [64];
uint64 *rook_magic_tables   [64];
